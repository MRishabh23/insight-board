...
import type { IssueColumnType } from "@/utils/types/common";
...
export function IssueTable({ ...props }) {
	...
	const columns: ColumnDef<IssueColumnType>[] = [
		{
			...
			cell: ({ row }) => (
				<TableCellCustom className="font-semibold">{row.original.value.issueKey}</TableCellCustom>
			),
			meta: {
		...
		},
		{
			...
			cell: ({ row }) => {
				...
				if (severity === "MEDIUM")
					severityColor = commonClass + " bg-yellow-50 border-yellow-500 text-yellow-500";
				if (severity === "HIGH")
					severityColor = commonClass + " bg-orange-50 border-orange-500 text-orange-500";
	...
	if (issueQuery.data && !issueQuery.data?.success) {
		return (
			<div className="flex h-full flex-col items-center justify-center">
				<p className="font-bold text-xl">{issueQuery.data?.data}</p>
			</div>
		);
	}
